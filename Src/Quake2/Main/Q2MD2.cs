...
namespace Quake2
{
    public class CQ2MD2
    {
        private void BuildAliasModelBuffer(ref CModel.SModelMD2 _SModelMD2)
        {
            ...
            CProgram.gQ2Game.gCMain.ModelVertexDeclaration =
                new VertexDeclaration(CModel.ModelVertexFormat.VertexElements);
        ...
        public void R_DrawAliasModel(CModel.SEntities _CurrentEntity)
        {
            ...
            // set vertex buffer
            CProgram.gQ2Game.gGraphicsDevice.SetVertexBuffer(
                _CurrentEntity.Model.ModelMD2.vbModel[_CurrentEntity.ModelFrameSeqStart 
                + _CurrentEntity.ModelFrameCurrent]);

            // create model rotation and translation matrix
            wMatrix = Matrix.CreateFromYawPitchRoll(_CurrentEntity.Angles.Y, 
                _CurrentEntity.Angles.X, _CurrentEntity.Angles.Z);
            ...
            // bind new texture
            CProgram.gQ2Game.gEffect.Parameters["xTextureSkin"].SetValue(
                _CurrentEntity.Model.ModelMD2.skins[_CurrentEntity.ModelCurrentSkin]);

            foreach (EffectPass pass in CProgram.gQ2Game.gEffect.CurrentTechnique.Passes)
            {
                pass.Apply();
                CProgram.gQ2Game.gGraphicsDevice.DrawPrimitives(PrimitiveType.TriangleList,
                    0, _CurrentEntity.Model.ModelMD2.md2.num_tris);
            }
