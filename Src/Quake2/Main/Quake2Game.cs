...
namespace Quake2
{
    public class Quake2Game : Game
    {
        ...
        private Point GameBounds = new Point(800, 480);//(1280, 720); //window resolution
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                DrawRectangle(spriteBatch, new Rectangle(GameBounds.X / 2 - 4, 5 + (i * 20), 8, 8), 
                    Color.White * 0.2f);
            }

            //draw paddles
            DrawRectangle(spriteBatch, PaddleLeft, Color.White);
            DrawRectangle(spriteBatch, PaddleRight, Color.White);

            //draw ball
            Ball.X = (int)BallPosition.X;
            Ball.Y = (int)BallPosition.Y;
            DrawRectangle(spriteBatch, Ball, Color.White);

            //draw current game points
            for (int i = 0; i < PointsLeft; i++)
            {
                DrawRectangle(spriteBatch, new Rectangle((GameBounds.X / 2 - 25) - i * 12, 10, 10, 10), Color.White * 1.0f);
            }
            for (int i = 0; i < PointsRight; i++)
            {
                DrawRectangle(spriteBatch, new Rectangle((GameBounds.X / 2 + 15) + i * 12, 10, 10, 10), Color.White * 1.0f);
            }
